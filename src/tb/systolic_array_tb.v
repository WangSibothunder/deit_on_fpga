...
module systolic_array_tb;
    ...
    // --- 主测试流程 ---
    initial begin
        $dumpfile("src/rtl/systolic_array.vcd");
        $dumpvars(0, systolic_array_tb);
        
        $display("[TB] Loading .mem files from src/data/test_data/ ...");
        // 确保使用绝对路径或相对路径正确
        $readmemh("src/data/test_data/sa_weights.mem", mem_weights);
        $readmemh("src/data/test_data/sa_inputs.mem",  mem_inputs);
        $readmemh("src/data/test_data/sa_golden.mem",  mem_golden);
