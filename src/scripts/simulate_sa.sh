...
python src/scripts/gen_vectors_systolic.py

# 2. 编译 RTL (包含新的 TB)
echo "[2/4] Compiling Verilog..."
# 注意: 我们只编译阵列相关的底层文件，不包含 controller 和 core
iverilog -I src/rtl -o src/sim/systolic_sim.out \
    src/rtl/params.vh \
    src/rtl/pe.v \
    src/rtl/systolic_array.v \
    src/tb/systolic_array_tb.v
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    if command -v gtkwave &> /dev/null; then
        gtkwave src/sim/systolic_array.vcd &
    else
        echo "GTKWave not found in path. Please verify waveform manually: src/sim/systolic_array.vcd"
    fi
else
    gtkwave src/sim/systolic_array.vcd &
fi